...
public interface OgrenciRepository extends JpaRepository<Ogrenci, Long> {
    ...
    String QUESTION_3 = "SELECT DISTINCT REPLACE(sinif, ' ', '') FROM ogrenci";
    @Query(value = QUESTION_3, nativeQuery = true)
    List<String> findAllClasses();

    //Öğrenci tablosunda, 10A sınıfında olan kız öğrencileri listeleyiniz.
    String QUESTION_4 = "SELECT * FROM ogrenci WHERE REPLACE(sinif, ' ','') = '10A' AND cinsiyet = 'K'";
    ...
    String QUESTION_7 = "SELECT * FROM ogrenci WHERE REPLACE(sinif, ' ','') = '10A' ORDER BY ogrno DESC";
